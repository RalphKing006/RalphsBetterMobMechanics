...
public class GhastNormalAttack extends Attack {

  private int scheduler;

  public GhastNormalAttack(String name, BetterMob mob) {
    super(name, mob);
  }
  ...
  public void run(Player target) {
    if (target.getGameMode().equals(GameMode.SURVIVAL) || target.getGameMode().equals(GameMode.ADVENTURE)) {
      ...
      this.scheduler = Bukkit.getScheduler().scheduleSyncRepeatingTask((Plugin) MobAI.getInstance(), new Runnable() {
        boolean b = getMob().track(target.getLocation(), (float) MobAI.settings.configuration.getDouble("Ghast.NormalAttack.Speed"), 20.0D);
        ...
        public void run() {
          ...
          if (!this.b) {
            this.b = getMob().track(target.getLocation(), (float) MobAI.settings.configuration.getDouble("Ghast.NormalAttack.Speed"), 20.0D);
          } else {
            if (!target.isDead() && target.isOnline()) {
              ((CraftGhast) getMob().getEntity()).getHandle().a(((CraftPlayer) target).getHandle());
              getMob().runRandomAttack(target, MobAI.settings.configuration.getInt("Ghast.NormalAttack.NextAttackDelay"));
            } else {
              getMob().setBusy(false);
            }
            Bukkit.getScheduler().cancelTask(scheduler);
