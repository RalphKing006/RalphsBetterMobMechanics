...
public class SpiderSkyAttack extends Attack {

  private int taskId;

  public SpiderSkyAttack(String name, BetterMob mob) {
    super(name, mob);
  }
  ...
  public void run(Player target) {
    ...
    if (gameMode == GameMode.SURVIVAL || gameMode == GameMode.ADVENTURE) {
      ...
      taskId = Bukkit.getScheduler().scheduleSyncRepeatingTask(MobAI.getInstance(), new Runnable() {
        boolean b = getMob().track(target.getLocation(), (float) MobAI.settings.configuration.getDouble("Spider.SkyAttack.Speed"), 3.0D);
        ...
        public void run() {
          ...
          if (!this.b) {
            this.b = getMob().track(target.getLocation(), (float) MobAI.settings.configuration.getDouble("Spider.SkyAttack.Speed"), 3.0D);
          } else {
            ...
            target.damage(MobAI.settings.configuration.getDouble("Spider.SkyAttack.Damage"));
            getMob().runRandomAttack(target, MobAI.settings.configuration.getInt("Spider.SkyAttack.NextAttackDelay"));
            Bukkit.getScheduler().cancelTask(taskId);
          }
        }
      }, 0L, 5L);
